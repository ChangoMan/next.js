...
export default abstract class Server<ServerOptions extends Options = Options> {
   ...
   */
  protected readonly basePathNormalizer?: BasePathNormalizer
  ...
  private async handleRequestImpl(
    ...
    try {
      ...
      if (invoked) {
        ...
        if (parsedUrl.pathname !== invoked.pathname) {
          parsedUrl.pathname = invoked.pathname

          if (i18n && isLocaleRouteMatch(match)) {
            curPathname = i18n.pathname
          }

          addRequestMeta(req, '_nextRewroteUrl', curPathname)
          addRequestMeta(req, '_nextDidRewrite', true)
        }
        ...
        if (i18n) {
          if (isLocaleRouteMatch(match)) {
            // TODO: maybe we need to copy over other i18n properties?
            parsedUrl.query.__nextLocale = match.i18n.detectedLocale
